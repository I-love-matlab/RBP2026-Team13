...
class LineDetector(Node):
    ...
    def image_callback(self, msg):
        ...
        top_left, top_right, bottom_right, bottom_left = skeleton_cv.detect_monitor(image)
        ...
        rectified = skeleton_cv.rectify_monitor(image, top_left, top_right, bottom_right, bottom_left)
        ...
        line = skeleton_cv.detect_line(rectified)
        ...
        angle = skeleton_cv.calculate_angle(line)
    ...
    def detect_line(line):
        ...
        x1,y1=black_list[index1]
        x2,y2=black_list[index2]
        a=(y2-y1)/(x2-x1)
        b=y1-a*x1
    ...
    def calculate_angle(list):
        
        if 90>=np.arctan(list)>0:
            return np.arctan(list)-90
        else:
            return np.arctan(list)+90
